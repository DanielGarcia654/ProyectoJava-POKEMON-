...
public class tipoAgua extends Pokemon {
    ...
    * @param type Obtener el tipo de pokemon
     */
    public tipoAgua(String name, int health, int attackDamage, int defence, int speed, int state, String type) {
    ...
    public float setElementalMultiplier(Player jugador, String enemyType){
        if ("Fuego".equals(enemyType) || "Tierra".equals(enemyType) || "Roca".equals(enemyType)){
            System.out.println("¡¡¡Ataque super efectivo!!!");
            return elementalMultiplier = (float) 2.0;
        }
        
        if ("Agua".equals(enemyType) || "Pasto".equals(enemyType) || "Roca".equals(enemyType)){
